...
import { Disc, Play, Pause, RefreshCw } from "lucide-react";
...
export default function BlackHoleBackground() {
  ...
  useEffect(() => {
    ...
    const DISK_INNER_RADIUS = BLACK_HOLE_RADIUS + 0.2; // 1.5
    ...
    const starCount = 80000; // Optimized for full performance inside browser
    ...
    // 8. Black Hole Singularity Sphere
    ...
    // 9. Accretion Disk (Ring Geometry)
  ...
  }, [autoRotate, controlsInstance]);

  return (
    <div className="absolute inset-0 w-full h-full overflow-hidden select-none z-0">
      <div ref={containerRef} className="w-full h-full absolute inset-0 bg-[#000002]" />
      
      {/* Interactive controls button */}
      <div className="absolute bottom-4 right-4 z-10 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10 hover:border-white/20 transition-all flex items-center gap-2 text-[10px] uppercase font-bold tracking-wider text-neutral-300 hover:text-white select-none">
        <button
          onClick={() => setAutoRotate(!autoRotate)}
          className="flex items-center gap-1.5 cursor-pointer outline-none"
          title={autoRotate ? "Désactiver la rotation automatique" : "Activer la rotation automatique"}
        >
